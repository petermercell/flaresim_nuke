...
// which is what manifests as Nuke crashing the moment _validate(true)
// runs (i.e. when the user connects an input or opens Properties).
// std::atomic_flag is a pure compiler intrinsic — no CRT dependency at all,
// so this is safe across CRT versions on every platform.
// ---------------------------------------------------------------------------
struct SpinLock {
...
public:
    ...
    //   0 = Manual XY    — single source at manual_xy_
    //   1 = Auto Detect  — scan input 0 RGB for bright pixels
    int         source_mode_;
    ...
    void rebuild_surf_ui()
    {
        ...
        if (lens_.surfaces.empty()) {
            ...
            // Use Knob::visible(bool) — NOT set_flag(HIDDEN).  The flag only
            // controls initial widget creation in knobs(); once the panel is
            // built, flipping the flag does nothing to the live widget.
            // visible(bool) manipulates the Qt widget directly AND updates
            // the flag, so it works whether the panel is open or not.
            for (int k = 0; k < MAX_SURFS_UI; ++k) {
                Knob* kt = knob(s_surf_knob_names.toggle_names[k]);
                if (kt) kt->visible(false);
                Knob* kg = knob(s_surf_knob_names.gain_names[k]);
                if (kg) kg->visible(false);
                Knob* kc = knob(s_surf_knob_names.color_names[k]);
                if (kc) kc->visible(false);
                Knob* kox = knob(s_surf_knob_names.offx_names[k]);
                if (kox) kox->visible(false);
                Knob* koy = knob(s_surf_knob_names.offy_names[k]);
                if (koy) koy->visible(false);
                Knob* ks = knob(s_surf_knob_names.scale_names[k]);
                if (ks) ks->visible(false);
            }
            return;
        }
        ...
        for (int k = 0; k < nsurf; ++k)
        {
            ...
            Knob* kc = knob(s_surf_knob_names.color_names[k]);
            if (kc) kc->visible(true);
            Knob* kox = knob(s_surf_knob_names.offx_names[k]);
            if (kox) kox->visible(true);
            Knob* koy = knob(s_surf_knob_names.offy_names[k]);
            if (koy) koy->visible(true);
            Knob* ks = knob(s_surf_knob_names.scale_names[k]);
            if (ks) ks->visible(true);
        }
    ...
    int knob_changed(Knob* k) override
    {
        ...
        auto sync_source_mode_enabled = [this]() {
            const bool is_manual = (source_mode_ == 0);
    ...
    void do_compute()
    {
        ...
        {
            // ================================================================
            // Manual XY mode — single source at user-defined XY knob
            // ================================================================
        ...
        else if (source_mode_ == 1)
        {
            ...
            const int w  = x1 - x0;
            const int h  = y1 - y0;
            const int dw = std::max(1, (w + ds - 1) / ds);
            const int dh = std::max(1, (h + ds - 1) / ds);
