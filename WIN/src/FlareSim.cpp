...
// Set to 1 to enable verbose debug output to stderr (crash diagnosis).
#define FLARESIM_DEBUG 0

#if FLARESIM_DEBUG
#define DBG(...) do { fprintf(stderr, "FlareSim DBG: "); fprintf(stderr, __VA_ARGS__); fprintf(stderr, "\n"); fflush(stderr); } while(0)
#else
#define DBG(...) ((void)0)
#endif
...
// ---------------------------------------------------------------------------
// Simple spin-lock that satisfies the BasicLockable concept (works with
// std::lock_guard).  We use this instead of std::mutex because Nuke 14.x
// ships MSVC 2019 CRT while the plugin may be built with MSVC 2022.  The
// different CRT versions have incompatible std::mutex internals (Windows
// CRITICAL_SECTION layout), causing an instant crash on try_lock().
// std::atomic_flag is a pure compiler intrinsic — no CRT dependency at all.
// ---------------------------------------------------------------------------
struct SpinLock {
...
public:
    ...
    // See SpinLock definition above for details.
    // ========================================================================
        ...
        , outside_source_falloff_(0.0f)
    {
        ...
        DBG("FlareSim constructor done (node=%p, this=%p, sizeof=%zu, &mutex=%p, mutex_offset=%zu)",
            (void*)node, (void*)this, sizeof(FlareSim),
            (void*)&compute_mutex_, (size_t)((char*)&compute_mutex_ - (char*)this));
    }
    ...
    {
        DBG("_validate(for_real=%d)", for_real ? 1 : 0);
        copy_info();
        ...
        if (!for_real) { DBG("  not for_real, return"); return; }
        ...
        const int w  = x1 - x0;
        const int h  = y1 - y0;
        DBG("  bbox: %d,%d -> %d,%d  (%dx%d)", x0, y0, x1, y1, w, h);
        if (w <= 0 || h <= 0) { DBG("  empty bbox, return"); return; }

        // Load lens file (only when the path changes)
        const std::string cur_lens(lens_file_ ? lens_file_ : "");
        DBG("  lens_file: '%s'", cur_lens.c_str());
        if (cur_lens != last_lens_file_)
        {
            if (!cur_lens.empty())
            {
                ...
                if (!lens_.load(cur_lens.c_str()))
                    ...
                    DBG("  lens loaded: %d surfaces", lens_.num_surfaces());
            }
            last_lens_file_ = cur_lens;
            DBG("  rebuild_surf_ui()...");
            rebuild_surf_ui();
            DBG("  rebuild_surf_ui() done");
        }

        DBG("  entering lock block...");
        {
            std::lock_guard<SpinLock> lock(compute_mutex_);
            DBG("  lock acquired");
            if (!pending_cuda_error_.empty()) {
                DBG("  pending CUDA error: %s", pending_cuda_error_.c_str());
                std::string msg;
            ...
            needs_compute_ = true;
            DBG("  pending state set: frame=%d, %dx%d, fmt=%dx%d",
                pending_frame_, w, h, pending_fmt_w_, pending_fmt_h_);
        }
        DBG("_validate() done");
    }
    ...
    {
        PROF_DECL;
        DBG("do_compute() enter");

        const int w = pending_w_;
        const int h = pending_h_;
        const size_t npx = (size_t)w * h;
        DBG("  buffer size: %d x %d = %zu px", w, h, npx);

        PROF_BEGIN("alloc_pinned");
        DBG("  calling ensure_pinned_output_fp16(npx=%zu)...", npx);
        void* pinned = ensure_pinned_output_fp16(gpu_cache_, npx);
        DBG("  ensure_pinned returned %p", pinned);
        ...
        if (lens_.surfaces.empty()) { DBG("  no surfaces, early out"); zero_buffers(); return; }
        ...
        if (sources.empty()) { DBG("  no sources, early out"); zero_buffers(); return; }

        DBG("  sources: %d, sensor_half: %.3f x %.3f", (int)sources.size(), 
            lens_.focal_length * std::tan(fov_h * 0.5f),
            lens_.focal_length * std::tan(fov_v * 0.5f));
        ...
        if (active_pairs.empty()) { DBG("  no active pairs, early out"); zero_buffers(); return; }
        ...
        DBG("  launching ghost kernel: %d pairs, %d sources, buf %dx%d, fmt %dx%d",
            (int)active_pairs.size(), (int)sources.size(), w, h,
            pending_fmt_w_, pending_fmt_h_);
        DBG("  cfg: ray_grid=%d, gain=%.1f, spec=%d, jitter=%d, clip=%.2f, knee=%.2f",
            cfg.ray_grid, cfg.gain, cfg.spectral_samples,
            cfg.spectral_jitter, cfg.highlight_clip, cfg.highlight_knee);
        ...
        PROF_END();
        DBG("  ghost kernel done. error=%s", pending_cuda_error_.empty() ? "(none)" : pending_cuda_error_.c_str());

        DBG("  launching blur_alpha_readback...");
        PROF_BEGIN("gpu_blur_alpha_readback");
        {
            ...
            DBG("  blur radius=%d, passes=%d", radius, ghost_blur_passes_);
        ...
        PROF_END();
        DBG("  blur_alpha_readback done. error=%s", pending_cuda_error_.empty() ? "(none)" : pending_cuda_error_.c_str());
                ...
                ChannelMask channels, Row& row) override
    {
        ...
        if (first_call) { DBG("engine() first call: y=%d x=%d r=%d", y, x, r); first_call = false; }
#endif
        ...
        const int cur_frame = (int)outputContext().frame();
        {
            ...
            if (needs_compute_ && pending_frame_ == cur_frame) {
                DBG("engine() triggering do_compute for frame %d", cur_frame);
                needs_compute_ = false;
                do_compute();
                cache_frame_ = cur_frame;
                DBG("engine() do_compute returned");
            }
        }
...
namespace {
    struct DllLoadDebug {
        DllLoadDebug() {
            fprintf(stderr, "FlareSim DBG: DLL loaded — FlareSim plugin initializing\n");
            fflush(stderr);
        }
    } _dll_load_dbg;
}
...
static Iop* build(Node* node) {
    DBG("build() called — creating FlareSim node");
    return new FlareSim(node);
}
