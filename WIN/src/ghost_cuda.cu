...
// Debug output — matches FLARESIM_DEBUG in FlareSim.cpp.
// Set to 1 for crash diagnosis.
#define FLARESIM_DEBUG 0

#if FLARESIM_DEBUG
#define DBG(...) do { fprintf(stderr, "ghost_cuda DBG: "); fprintf(stderr, __VA_ARGS__); fprintf(stderr, "\n"); fflush(stderr); } while(0)
#else
#define DBG(...) ((void)0)
#endif

// CUDA error check with debug output
#define CUDA_DBG_CHK(call) do { \
    cudaError_t _e = (call); \
    if (_e != cudaSuccess) { \
        fprintf(stderr, "ghost_cuda CUDA ERROR at %s:%d — %s: %s\n", \
...
{
    DBG("ensure_pinned_output_fp16(n_px=%zu, cached=%zu, ptr=%p)",
        n_px, cache.h_pinned_elems, cache.h_pinned);

    // Check CUDA device is available
    {
        int dev = -1;
        cudaError_t e = cudaGetDevice(&dev);
        DBG("  cudaGetDevice: dev=%d, err=%s", dev, cudaGetErrorString(e));
        if (e != cudaSuccess) {
            fprintf(stderr, "FlareSim: no CUDA device available: %s\n", cudaGetErrorString(e));
            // Fall back to malloc
            std::size_t bytes = 4 * n_px * sizeof(uint16_t);
            if (cache.h_pinned) { free(cache.h_pinned); cache.h_pinned = nullptr; }
    ...
    std::size_t bytes = 4 * n_px * sizeof(uint16_t);
    DBG("  cudaHostAlloc(%zu bytes)...", bytes);
    ...
    DBG("  result: ptr=%p", cache.h_pinned);
    cache.h_pinned_elems = n_px;
    return cache.h_pinned;
}
...
struct DVec3 {
    float x, y, z;
    __device__ __host__ DVec3() : x(0), y(0), z(0) {}
    __device__ __host__ DVec3(float x_, float y_, float z_) : x(x_), y(y_), z(z_) {}
};
...
__device__ __forceinline__ DVec3 dv_normalize(DVec3 v)
{
    ...
    return DVec3(v.x*inv, v.y*inv, v.z*inv);
}
...
struct DTraceResult {
    DVec3 position; float weight; bool valid;
    __device__ __host__ DTraceResult() : position(), weight(0), valid(false) {}
    __device__ __host__ DTraceResult(DVec3 p, float w, bool v) : position(p), weight(w), valid(v) {}
};
...
{
    DBG("launch_ghost_cuda: pairs=%d, sources=%d, buf=%dx%d, fmt=%dx%d",
        (int)active_pairs.size(), (int)sources.size(), width, height, fmt_w, fmt_h);

    if (active_pairs.empty() || sources.empty()) { DBG("  empty pairs/sources, return"); return; }
    ...
    // before we hit a cryptic allocator error.
    {
        ...
        DBG("  cudaGetDeviceCount: count=%d, err=%s", device_count, cudaGetErrorString(ce));
    ...
    DBG("  allocating GPU buffers: n_surfs=%d, n_pairs=%d, n_src=%d, n_grid=%d, n_spec=%d, n_px=%zu",
        n_surfs, n_pairs, n_src, n_grid, n_spec, n_px);
    ...
    {
        DBG("  growing output buffers to %zu floats (%.1f MB per channel)", n_px, n_px*4.0/1048576.0);
...
#define GPU_CHK(call) \
    ...
    // ---- Launch ----
    DBG("  ghost_kernel launch: grid=(%u,%u,1), block=(%d,1,1), n_spec=%d, n_grid=%d",
        (unsigned)(n_pairs * n_src), (unsigned)grid_blocks, BLOCK_SIZE, n_spec, n_grid);
    ...
    GPU_CHK(cudaDeviceSynchronize());
    {
        ...
        DBG("  ghost_kernel done. post-sync error=%s", cudaGetErrorString(post_err));
    }
...
{
    DBG("launch_blur_alpha_readback_async: %dx%d, blur=%d×%d, clip=%.2f, knee=%.2f, metric=%d",
        w, h, blur_radius, blur_passes, highlight_clip, highlight_knee, highlight_metric);

    const size_t n_px       = (size_t)w * h;
    const size_t n_bytes_f  = n_px * sizeof(float);
    const size_t n_bytes_h  = n_px * sizeof(__half);
    if (n_px == 0) { DBG("  n_px==0, return"); return; }

    auto report = [&](cudaError_t e, const char* site) {
        fprintf(stderr, "FlareSim async error at %s -- %s\n",
                site, cudaGetErrorString(e));
        DBG("  ASYNC ERROR at %s: %s", site, cudaGetErrorString(e));
        if (out_error && out_error->empty()) {
            char buf[256];
    ...
    DBG("  creating/reusing streams...");
    if (!cache.compute_stream) {
        cudaStream_t cs;
        cudaError_t e = cudaStreamCreate(&cs);
        DBG("  cudaStreamCreate(compute): %s", cudaGetErrorString(e));
    ...
    if (!cache.copy_stream) {
        cudaStream_t cs;
        cudaError_t e = cudaStreamCreate(&cs);
        DBG("  cudaStreamCreate(copy): %s", cudaGetErrorString(e));
    ...
    DBG("  blur: do_blur=%d, do_clip=%d", do_blur ? 1 : 0, do_clip ? 1 : 0);
    if (do_blur)
    {
    ...
    if (do_clip)
    {
        DBG("  soft_clip: clip=%.2f, knee=%.3f, metric=%d", highlight_clip, highlight_knee, highlight_metric);
    ...
    // ---- Wait for everything ----
    DBG("  synchronizing streams...");
    cudaStreamSynchronize(copy);
    cudaStreamSynchronize(comp);
    DBG("  streams synced. checking final error...");
    {
        cudaError_t final_err = cudaGetLastError();
        DBG("  final cudaGetLastError: %s", cudaGetErrorString(final_err));
    }

    for (int i = 0; i < 4; ++i)
        cudaEventDestroy(ev[i]);
    DBG("launch_blur_alpha_readback_async done");
}
