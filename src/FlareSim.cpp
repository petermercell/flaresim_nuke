...
public:
    ...
    // Stored as uint16_t* (same layout as __half, avoids CUDA headers).
    ...
    void do_compute()
    {
        ...
        // Grow the persistent FP16 pinned staging in gpu_cache_.
        // DMA writes FP16 directly here; engine() converts per-scanline.
        ...
        // Lambda: zero CPU buffers for early-out paths (no GPU work).
        // FP16 zero is 0x0000, so memset(0) works correctly.
        auto zero_buffers = [&]() {
        ...
        if (cnt > 0)
        {
            ...
            if (luma >= threshold_)
            {
                // Apply source intensity boost — restores the true HDR
                // brightness that the camera sensor clipped.
                // Internal ×1000 so user-facing values stay friendly (8 → 8000).
        ...
        // filter_ghost_pairs traces probe rays — expensive but deterministic
        // for a given lens + sensor geometry.
        ...
        launch_ghost_cuda(lens_, active_pairs, area_boosts, sources,
                          ...
                          nullptr, nullptr, nullptr,  // skip_readback: CPU ptrs unused
                          w, h,
        ...
        // Async pipeline: blur → alpha → readback, overlapping readback
        // of finished channels with blur of remaining ones.
        // DMA writes directly into pinned output cache (ghost_r_ etc).
        {
            int radius = 0;
