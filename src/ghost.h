...
struct GhostConfig
{
    ...
    // Spectral jitter: randomise each ray's wavelength within its spectral
    // bin.  Smooths the hard colour boundaries between discrete wavelength
    // samples at no extra ray-trace cost.  0 = off, 1 = on.
    int spectral_jitter = 0;

    // Per-frame seed for spectral jitter noise pattern.  Animate to get
    // temporal variation (useful for temporal denoising / accumulation).
    int spectral_jitter_seed = 0;

    // Spectral jitter noise scale: multiplier on the random wavelength offset.
    // 1.0 = full bin width (default).  0.5 = half bin, subtle.
    // 2.0 = double bin, aggressive blending across bins.
    float spectral_jitter_scale = 1.0f;

    // Highlight compression: soft-clip applied after blur, before alpha.
    // clip = 0 disables.  Positive values set the output ceiling.
    // knee = transition sharpness (0 = hard clip, 1 = very soft rolloff).
    float highlight_clip = 0.0f;
    float highlight_knee = 0.5f;
    // 0 = Value (max RGB), 1 = Luminance (Rec.709), 2 = Lightness (cube root)
