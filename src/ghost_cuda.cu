    ...
    float           fmt_y0_in_buf)      // format origin y within buffer
{
    ...
    if (active)
    {
        ...
        for (int s = 0; s < n_spec; ++s)
        {
            ...
            // For the classic 3-sample mode (R-only, G-only, B-only)
            // this skips up to 2/3 of traces for monochromatic sources.
    ...
    GpuBufferCache& cache, std::string* out_error)
{
    ...
    // The caller's cpu_r/g/b/a pointers are actually uint16_t* into
    // the FP16 pinned buffer, cast to float* by the caller for API compat.
    // We reinterpret them as __half* for the async DMA.
    __half* h_dst[4] = {
    ...
    // ---- Per-channel: blur(FP32) → convert(FP16) → DMA(FP16) ----
    for (int ch = 0; ch < 3; ++ch)
    {
        ...
        // Convert FP32 → FP16 on compute stream.
        fp32_to_fp16_kernel<<<cvt_grid, CVT_BLK, 0, comp>>>(
            d_ch[ch], d_fp16[ch], (int)n_px);

        // Signal: this channel's FP16 data is ready.
        cudaEventRecord(ev[ch], comp);

        // Async DMA: FP16 device → FP16 pinned host (half the bytes!).
        cudaStreamWaitEvent(copy, ev[ch], 0);
        cudaMemcpyAsync(h_dst[ch], d_fp16[ch], n_bytes_h,
                        cudaMemcpyDeviceToHost, copy);
    }

    // ---- Alpha: FP32 compute → FP16 convert → DMA ----
    {
        alpha_kernel<<<cvt_grid, CVT_BLK, 0, comp>>>(
            cache.d_out_r, cache.d_out_g, cache.d_out_b,
            cache.d_out_a, (int)n_px);
        fp32_to_fp16_kernel<<<cvt_grid, CVT_BLK, 0, comp>>>(
            cache.d_out_a, d_fp16[3], (int)n_px);
    ...
    if (h_dst[3])
    {
        ...
        cudaMemcpyAsync(h_dst[3], d_fp16[3], n_bytes_h,
                        cudaMemcpyDeviceToHost, copy);
    }
