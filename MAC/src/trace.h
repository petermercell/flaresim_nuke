...
// Intersect a ray with a single lens surface (sphere, cylinder, or flat).
...
//
// Dispatches on Surface::surface_type:
//   SURF_SPHERICAL   - ray-sphere intersection (legacy default).
//   SURF_CYLINDER_X  - cylinder, axis along X (curves in YZ).
//   SURF_CYLINDER_Y  - cylinder, axis along Y (curves in XZ).
//   SURF_TORIC       - Two-radii toric surface (`radius` = Rx in XZ,
//                      `radius_y` = Ry in YZ).  Newton-Raphson on the
//                      degree-4 implicit polynomial.
// A surface with surf.radius == 0 is treated as a plane regardless of type.
...
// ---- Toric Newton-Raphson profiling (M7) -------------------------------
// Available only when trace.cpp was compiled with -DFLARESIM_TORIC_PROFILE.
// Reports the number of toric intersection calls, average and max Newton
// iterations needed to converge, and the count of misses (vignettes / TIR
// / aperture clips).  Use the data to tune intersect_toric's MAX_ITER for
// your real lens prescriptions.
