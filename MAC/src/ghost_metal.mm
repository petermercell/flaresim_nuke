...
bool metal_init(MetalBufferCache& cache, std::string* out_error)
{
    ...
    @autoreleasepool {
        ...
        if (!pso_ghost || !pso_alpha || !pso_fp16 || !pso_clear ||
            !pso_blur_h || !pso_blur_v || !pso_blur_hn)
            return false;
    ...
    std::string*      out_error)
{
    ...
    // ---- Per-channel: blur(FP32) → FP16 convert ----
    for (int ch = 0; ch < 3; ++ch)
    {
            ...
            for (int p = 0; p < blur_passes; ++p)
            {
                // Horizontal blur: blurA → blurB
                {
                    ...
                    if (w <= MAX_PREFIX_W) {
                        ...
                        // Threadgroup memory for prefix sum
        ...
        // FP32 → FP16 conversion
        {
            ...
            [enc setBuffer:d_ch[ch] offset:0 atIndex:0];
    ...
    // ---- Alpha: compute from RGB, then convert to FP16 ----
    {
        id<MTLComputeCommandEncoder> enc = [cmdBuf computeCommandEncoder];
    ...
    }
    {
        id<MTLComputeCommandEncoder> enc = [cmdBuf computeCommandEncoder];
        [enc setComputePipelineState:pso_fp16];
        [enc setBuffer:d_alpha offset:0 atIndex:0];
        [enc setBuffer:d_fp16  offset:(3 * n_px * sizeof(uint16_t)) atIndex:1];
        [enc setBuffer:paramN  offset:0 atIndex:2];
        int blocks = ((int)n_px + 255) / 256;
        [enc dispatchThreadgroups:MTLSizeMake(blocks, 1, 1)
            threadsPerThreadgroup:MTLSizeMake(256, 1, 1)];
