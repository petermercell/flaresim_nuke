...
// Surface geometry type — wire-compatible with SurfaceType in lens.h.
// Values are fixed (0..3) so host and device byte layouts agree.
constant int SURF_SPHERICAL  = 0;
constant int SURF_CYLINDER_X = 1;   // axis ‖ X, curves in YZ (radius = Ry)
constant int SURF_CYLINDER_Y = 2;   // axis ‖ Y, curves in XZ (radius = Rx)
constant int SURF_TORIC      = 3;   // two radii: radius=Rx, radius_y=Ry

// Lens surface (POD, matches the GPUSurface POD in ghost_metal.mm).
// Layout MUST stay in lockstep with that struct — the host uploads raw bytes.
struct Surface
{
    float radius;
    float thickness;
    ...
    float z;
    int   surface_type;   // SurfaceType (M3 anamorphic dispatch)
    float radius_y;       // toric only (= Ry); 0 for spherical / cylinder
};
...
// ---- Wang hash (fast 32-bit → uniform float) ----

inline uint d_wang_hash(uint s)
{
    s = (s ^ 61u) ^ (s >> 16u);
    s *= 9u;
    s ^= s >> 4u;
...
inline float d_hash_float(uint s)   // → [0, 1)
{
...
inline float d_cie_r(float l)
{
    float a = (l - 600.0f) / 70.0f, b = (l - 450.0f) / 30.0f;
    return max(0.0f, 0.63f * exp(-0.5f*a*a) + 0.22f * exp(-0.5f*b*b));
}
inline float d_cie_g(float l)
{
    float a = (l - 545.0f) / 55.0f;
    return max(0.0f, exp(-0.5f*a*a));
}
inline float d_cie_b(float l)
{
...
//
// Mirrors the CPU intersect_surface() in trace.cpp and the CUDA
// d_intersect_surface() in ghost_cuda.cu: a flat shortcut at the top
// (applies regardless of surface_type), then a switch on surface_type
// that dispatches to a per-shape helper.
//
// Warp/SIMD-divergence note: every thread in a SIMD-group traces through the
// SAME surface at each step of the trace loop, so all threads land in the
// same switch arm together — no intra-SIMD divergence.

// ---- Spherical: centre of curvature at C = (0, 0, surf.z + R) -----------
inline bool d_intersect_spherical(DRay ray, thread const Surface& surf,
...
// ∇F has zero X-component, so reflections preserve ray.dir.x → fan out
// purely in YZ → horizontal streaks on the sensor (anamorphic signature).
inline bool d_intersect_cylinder_x(DRay ray, thread const Surface& surf,
                                    thread DVec3& hit, thread DVec3& norm)
{
    float R  = surf.radius;
    float cz = surf.z + R;
...
// ---- Cylinder, axis along Y — curves in XZ.  surf.radius = Rx -----------
// X↔Y mirror of intersect_cylinder_x.
inline bool d_intersect_cylinder_y(DRay ray, thread const Surface& surf,
                                    thread DVec3& hit, thread DVec3& norm)
{
    float R  = surf.radius;
    float cz = surf.z + R;
...
// ---- Toric: two-radii surface (M3-Metal port of CPU intersect_toric) ----
//
...
//
// Newton-Raphson from the apex-plane intersection (z' = 0).  Falls back to
// the spherical helper when |W| ≈ 0 (doubled-root quartic ⇒ ∇F vanishes on
// the surface).  See trace.cpp / ghost_cuda.cu for the full derivation.
//
...
// a SIMD-group traces the same surface so loop-iteration counts stay
// coherent.  MAX_ITER=16 mirrors the CUDA path; profile via the CPU's
// FLARESIM_TORIC_PROFILE flag before re-tuning for your prescriptions.
inline bool d_intersect_toric(DRay ray, thread const Surface& surf,
                               thread DVec3& hit, thread DVec3& norm)
{
    ...
    const float scale  = max(abs(Rx), abs(Ry));
    const float scale2 = scale * scale;
    const float TOL_F  = scale2 * scale2 * 1e-8f;
    const float STEP_CAP = abs(t) + scale;
    const float W2  = W  * W;
    ...
    for (int iter = 0; iter < MAX_ITER; ++iter)
    {
        ...
        if (abs(F) < TOL_F) { converged = true; break; }

        const float dF = 2.0f*G*dG - 4.0f*W2*dQ;
        if (abs(dF) < 1e-30f) return false;
        ...
        t -= step;
        if (!(t > 1e-6f)) return false;
        if (abs(step) < 1e-7f) { converged = true; break; }
    }
    if (!converged)
    {
        ...
        const float Q  = px*px + Az*Az;
        const float G  = Q + W2 + py*py - Ry2;
        if (abs(G*G - 4.0f*W2*Q) > TOL_F * 1e3f) return false;
    }
...
inline bool d_intersect_surface(DRay ray, threadgroup const Surface& surf,
                                 thread DVec3& hit, thread DVec3& norm)
{
    // Flat shortcut — applies regardless of surface_type.  A surface with
    // no curvature is a plane no matter what type field says.
    ...
    // Promote threadgroup surface to a thread-local copy so the per-shape
    // helpers can take a regular `const Surface&` (default `thread`
    // address space).  The toric helper also needs to mutate a copy for
    // its degenerate-W spherical fallback, which would not be permitted on
    // a `threadgroup const Surface&` directly.
    Surface s = surf;

    switch (s.surface_type)
    {
        case SURF_SPHERICAL:   return d_intersect_spherical (ray, s, hit, norm);
        case SURF_CYLINDER_X:  return d_intersect_cylinder_x(ray, s, hit, norm);
        case SURF_CYLINDER_Y:  return d_intersect_cylinder_y(ray, s, hit, norm);
        case SURF_TORIC:       return d_intersect_toric    (ray, s, hit, norm);
        default:
            return false;
    }
}
    ...
    uint3                            tg_size       [[threads_per_threadgroup]])
{
    ...
    for (int s = 0; s < params.n_spec; ++s)
    {
        ...
        if (params.spectral_jitter && params.spec_bin_width > 0.1f)
        {
            uint ray_id = uint(ps_idx * params.n_grid + grid_idx);
            uint seed = d_wang_hash(ray_id * 31u + uint(s) * 7919u + params.spec_jitter_seed);
            float offset = d_hash_float(seed) - 0.5f;

            trace_lambda = spec.lambda + offset * params.spec_bin_width;
            trace_lambda = clamp(trace_lambda, 380.0f, 720.0f);

            use_rw = d_cie_r(trace_lambda) / max(params.cie_norm_r, 1e-9f);
            use_gw = d_cie_g(trace_lambda) / max(params.cie_norm_g, 1e-9f);
            use_bw = d_cie_b(trace_lambda) / max(params.cie_norm_b, 1e-9f);
        }
...
// ===========================================================================
// Luminance-preserving soft-clip kernel
//
// Matches the CUDA version and AFXToneMap behaviour:
//   metric 0 = Value     (max of R, G, B)
//   metric 1 = Luminance (Rec.709)
//   metric 2 = Lightness (cube root of Rec.709 luminance)
//
// clip_val : maximum output (asymptotic ceiling)
// knee     : transition sharpness (0 = soft, 1 = hard)
//            — matches AFXToneMap convention
// ===========================================================================

struct SoftClipParams
{
    float clip;
    float knee;
    int   metric;
...
kernel void soft_clip_kernel(
    device       float* d_r      [[buffer(0)]],
    device       float* d_g      [[buffer(1)]],
    device       float* d_b      [[buffer(2)]],
    constant     SoftClipParams& p [[buffer(3)]],
    uint         tid              [[thread_position_in_grid]])
{
    if ((int)tid >= p.n_px) return;

    float r = d_r[tid], g = d_g[tid], b = d_b[tid];
    ...
    if (p.metric == 0) {
        ...
        m = (lum > 0.0f) ? pow(lum, 1.0f / 3.0f) : 0.0f;
    } else {
    ...
    // Soft-clip (AFXToneMap convention: knee=1 is hard, knee=0 is soft)
    float softness   = 1.0f - p.knee;
    float threshold  = p.clip * (1.0f - softness);
    float knee_range = p.clip * softness;
